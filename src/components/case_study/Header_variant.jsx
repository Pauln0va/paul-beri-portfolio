 ...
 const [isVisible, setIsVisible] = useState(true);
  ...
  return (
    <>
      <header
        ref={headerRef}
        className={`fixed top-0 left-0 w-full transform bg-gradient-to-b from-black/40 to-transparent from-20% transition-transform duration-300 z-[1000] ${
          isVisible ? "translate-y-0" : "-translate-y-full"
        }`}
      >
        {/* Main header content */}
        <div className="flex justify-between sm:justify-around items-center gap-8 px-8">
          <div className="flex flex-col gap-2 justify-center items-center py-2 px-8">
            <div className="flex justify-center items-center rounded-full p-2 sm:p-4">
              <Link to='/'>
                <img src="/pb2.svg" alt="profile" className="h-10 sm:h-12 aspect-square cursor-pointer" />
              </Link>
            </div>
          ...
          >
            <img className={`${isActive ? 'hidden' : "flex"} active:rotate-180 transition-all duration-300`} src="/icons/menu-light.svg" alt="menu" />
            <img className={`${isActive ? 'flex' : 'hidden'} active:rotate-180 transition-all duration-300`} src="/icons/close-light.svg" alt="close" />
            {isActive &&
            <div className="absolute -bottom-28 bg-white rounded-md -right-2 flex flex-col shadow-lg w-32">
              ...
              <NavLink onClick={handleLinkClick} to='/journey'>
          ...
          <div className="hidden sm:flex gap-8">
            <NavLink to='/'>
              <p className="text-sm cursor-pointer text-white hover:text-gray-200 transition-colors duration-200">Home</p>
            </NavLink>
            <NavLink to='/journey'>
              <p className="text-sm cursor-pointer text-white hover:text-gray-200 transition-colors duration-200">Journey</p>
            </NavLink>
            <NavLink to='/about'>
              <p className="text-sm cursor-pointer text-white hover:text-gray-200 transition-colors duration-200">About</p>
            </NavLink>
          </div>
